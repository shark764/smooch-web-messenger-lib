...
let cxengageWebAppId, cxengageRegion, cxengageProactiveChatMessage, cxengagePrechatCaptureGreetingText, cxengagePrechatCaptureConfirmationText;
...
if (cxengageWebScript) {
  ...
  if (!cxengageWebAppId) {
    throw new Error('web-app-id not passed into script as data attribute. data-cxengage-web-app-id: ' + cxengageWebAppId);
  }
} else {
  ...
  if (!cxengageWebAppId) {
    throw new Error('appId not passed in as URL params. appId: ' + cxengageWebAppId);
  }
}
